...
napp.controller("insti-controller", function($scope,$location,$http){
	...
	// load the temp files
	$http.get('../database/temp_data.json').then(function(response){
		console.log(response.data);
		instinct_profile = response.data;
		console.log(instinct_profile.location_data);
	});
});
...
napp.controller('location_controller',function($scope,$http){
	...
	$scope.geo_loader = function(){
		...
		ipcRenderer.invoke('location_done',instinct_profile);
	};	
});
